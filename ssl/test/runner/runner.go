...
// When an existing test is converted to test SSL transfer,
// the test name is renamed with (|test.name| + |SSL_TRANSFER_SUFFIX|).
...
func runTest(dispatcher *shimDispatcher, statusChan chan statusMsg, test *testCase, shimPath string, mallocNumToFail int64) error {
	...
	// Process SSL transfer related messages generated by the bssl_shim.
	ssl_transfer_detect_msg := "Eligible for testing SSL transfer.\n"
	ssl_transfer_call_msg := "SSL transfer is going to be tested.\n"
	if strings.Contains(stderr, ssl_transfer_detect_msg) || strings.Contains(stderr, ssl_transfer_call_msg) {
		test_case_name := strings.Replace(test.name, SSL_TRANSFER_SUFFIX, "", -1)
		sslTransferHelper.AddNewCase(test_case_name)
		stderr = strings.Replace(stderr, ssl_transfer_detect_msg, "", -1)
		stderr = strings.Replace(stderr, ssl_transfer_call_msg, "", -1)
	}
